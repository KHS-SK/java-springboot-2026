...
public class StudyApplicationService {
    ...
    public void apply(StudyApplicationForm form) {
        ...
        if (approvedCount >= post.getMaxMembers()) {    // 게시글에 정해진 최대인원ㅇ르 넘어서서 신청할 수 없음
    ...
    // 260422. 스터디 신청 제약을 위해서 StudyPost도 전달
    public void approve(Long applicationId, StudyPost post) {

        // 스터디포스트 상태가 마감이면 더이상 승인 불가
        ...
        System.out.println("[승인 전] approvedCount=" + approvedCount + ", maxMembers=" + post.getMaxMembers());

        if (approvedCount >= post.getMaxMembers()) {
            throw new IllegalArgumentException("모집인원이 다 찼습니다.");
        }

        // 인원이 전부 신청승인 되고나면 스터디포스트 자체가 CLOSED로 변경
        // 신청 승인 로직
        ...
        System.out.println("[승인 후] approvedCount=" + approvedCount + ", maxMembers=" + post.getMaxMembers());
        // 정원이 다 찼으면 CLOSED
        if (approvedCount >= post.getMaxMembers()){
            // 상태만 바꾸는 메서드 추가
            this.studyPostMapper.updatePostStatus(post.getPostId(), "CLOSED");
            System.out.println(">>> 모집 마감 처리 완료 (CLOSED)");
        }
    }

    // 20260422. 스터디 거절 후 상태 변경 로직추가
