...
public class PageResponse<T> {
    ...
    private final int totalCount;   // 전체 게시글 수 / 예) 39개
    private final int currentPage;  // 사용자가 보고있는 현재 페이지 번호
    private final int size;         // 페이지당 게시글 수 10,
    private final int totalPage;    // 전체 페이지 수. 게시글이 32 / 10 = 3.2를 올림해서 4페이지
     ...
     */
    private final int startPage;    // 시작페이지, 화면에 보여 줄 시작페이지
    private final int endPage;      // 화면에 보여 줄 마지막 페이지
    ...
    public PageResponse(List<T> dtoList, int totalCount, int currentPage, int size) {
        this.dtoList = dtoList;         // 페이징 된 현재 페이지의 실제 게시글 리스트
