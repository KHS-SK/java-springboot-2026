...
public class BoardController {
    ...
    // BoardRepository와 같은 객체를 불러오면 Conbtroller에서 사용할 수 있도록 주입(DI)
    ...
    public String showList(Model model){
        ...
        model.addAttribute("boardList", boardList); // HTML로 보낼 모델데이터로 설정
    ...
    public String showDetail(Model model, ReplyForm replyForm, @PathVariable("bno") Long bno){
        // Board board = this.boardRepository.getById(bno); 다음 버전에선 서비스 안함 (deprecated)
    ...
    public String createBoard(@Valid BoardForm boardForm, BindingResult bindingResult) {
        if (bindingResult.hasErrors()){
            return "board_create";   // board_create.html다시 돌아감
        }
    ...
    public String deleteBoardOne(@PathVariable("bno") Long bno) {
